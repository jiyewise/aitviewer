...
#         self.use_pca = smpl_model_params.get('use_pca', False)
#         self.flat_hand_mean = smpl_model_params.get('flat_hand_mean', True)

#         # embed()
#         # bm = amass.load_body_model("./body_model/SMPLX_MALE.npz", model_type="smplx")

#         model_name = f"{model_type.upper()}_{gender.upper()}.npz"
#         bm_path = os.path.join(C.smplx_models, model_type, model_name)
...
#             self.NUM_BODY_JOINTS = 23 - 2
#             self.NUM_HAND_JOINTS = 15
#             self.NUM_JOINTS = self.NUM_BODY_JOINTS + 2 * self.NUM_HAND_JOINTS
#             self.num_pca_comps = 6
#         elif model_type == 'smplx':
#             self.NUM_BODY_JOINTS = 21
#             self.NUM_HAND_JOINTS = 15
#             self.NUM_FACE_JOINTS = 3
#             self.NUM_JOINTS = self.NUM_BODY_JOINTS + 2 * self.NUM_HAND_JOINTS + self.NUM_FACE_JOINTS
...
#         :return: The resulting vertices and joints.
#         """
#         assert poses_body.shape[1] == self.NUM_BODY_JOINTS*3

#         has_hands = hasattr(self, 'NUM_HAND_JOINTS')
#         has_face = hasattr(self, 'NUM_FACE_JOINTS')
#         if has_hands:
#             if self.use_pca:
#                 dof_per_hand = self.bm.num_pca_comps
#                 assert poses_left_hand is None or poses_left_hand.shape[1] == dof_per_hand
#                 assert poses_right_hand is None or poses_right_hand.shape[1] == dof_per_hand
#             else:
#                 dof_per_hand = self.NUM_HAND_JOINTS*3
#                 assert poses_left_hand is None or poses_left_hand.shape[1] == dof_per_hand
#                 assert poses_right_hand is None or poses_right_hand.shape[1] == dof_per_hand
#         else:
#             dof_per_hand = 0  # Silencing the might not be initialized warning.
...
#             poses_reye = torch.zeros([batch_size, 3]).to(dtype=poses_body.dtype, device=device)
#         if has_face and expression is None:
#             expression = torch.zeros([batch_size, self.num_expression_coeffs]).to(dtype=poses_body.dtype,
#                                                                                      device=device)
